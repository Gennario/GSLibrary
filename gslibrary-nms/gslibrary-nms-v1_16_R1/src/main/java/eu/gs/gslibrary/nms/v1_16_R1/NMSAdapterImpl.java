package eu.gs.gslibrary.nms.v1_16_R1;
...
public class NMSAdapterImpl implements NMSAdapter {
    ...
    public Object packetActionbarMessage(String text) {
        return new PacketPlayOutChat(s(text), ChatMessageType.GAME_INFO, null);
    }

    @Override
    public Object packetJsonMessage(String text) {
        return new PacketPlayOutChat(s(text), ChatMessageType.CHAT, null);
    }
    ...
    public double getEntityHeight(EntityType type) {
        return getEntityTypes(type).map((t) -> t.l().height).orElse(0f);
    }

    @Override
    public double getEntityWidth(EntityType type) {
        return getEntityTypes(type).map((t) -> t.l().width).orElse(0f);
    }
