package eu.gs.gslibrary.nms.v1_19_R1;
...
public class NMSAdapterImpl implements NMSAdapter {
    ...
    public Object packetActionbarMessage(String text) {
        return new ClientboundSetActionBarTextPacket(s(text));
    }

    @Override
    public Object packetJsonMessage(String text) {
        return new ClientboundSystemChatPacket(s(text), true);
    }
    ...
    public int getEntityTypeId(EntityType type) {
        return getEntityTypes(type).map(IRegistry.X::a).orElse(-1);
    }
    ...
    public Object packetSpawnEntityLiving(int eid, UUID id, EntityType type, Location l) {
        return packetSpawnEntity(eid, id, type, l);
    }
